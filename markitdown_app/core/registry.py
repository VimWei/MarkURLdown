...
def _zhihu_handler(payload: ConvertPayload, session, options: ConversionOptions) -> ConvertResult | None:
    ...
    try:
        fetched = fetch_zhihu_article(session, url)
...
def _wordpress_handler(payload: ConvertPayload, session, options: ConversionOptions) -> ConvertResult | None:
    """WordPress网站处理器 - 专门处理skywind.me等WordPress站点"""
    url = payload.value
    
    # 检查是否是WordPress站点（特别是skywind.me）
    if "skywind.me" not in url and not _is_wordpress_site(url):
        return None
...
HANDLERS: list[Handler] = [_weixin_handler, _zhihu_handler, _wordpress_handler]
