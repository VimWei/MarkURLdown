...
def test_download_single_image_success_and_dedup(tmp_path, monkeypatch):
    ...
    ok1, out1 = asyncio.run(_download_single_image(FakeSession(), 'https://x/1', str(path1), None, h2p, lock))
    # dedup: second download with same content should reuse first
    ok2, out2 = asyncio.run(_download_single_image(FakeSession(), 'https://x/2', str(path2), None, h2p, lock))
        ...
        async def get(self, *a, **k):
            class R:
                ...
                async def __aexit__(self, *a):
                    return False
            return R()
    ok, out = asyncio.run(_download_single_image(BadSession(), 'https://x', str(tmp_path/'a.img'), None, None, None))
    assert ok is False
