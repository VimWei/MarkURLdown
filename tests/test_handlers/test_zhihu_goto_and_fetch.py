...
def test_goto_target_and_prepare_content_calls_paths(monkeypatch):
    ...
    zh._goto_target_and_prepare_content(p, "https://www.zhihu.com/question/1/answer/2", on_detail=lambda s: None)
...
def test_fetch_zhihu_article_unknown_fallback_then_success(monkeypatch):
    ...
    monkeypatch.setattr(zh._generic, '_try_lightweight_markitdown', lambda url, session: zh.CrawlerResult(success=True, title='T', text_content='ok'*600))
