...
def test_weixin_try_playwright_crawler_success(monkeypatch):
	...
	page = DummyPage()
	monkeypatch.setattr(wx, "new_context_and_page", lambda b, apply_stealth=False: (DummyContext(page), page))
	monkeypatch.setattr(wx, "_goto_target_and_prepare_content", lambda p, url, cb=None: None)
	...
	monkeypatch.setitem(sys.modules, "playwright.sync_api", types.SimpleNamespace(sync_playwright=fake_sync))
