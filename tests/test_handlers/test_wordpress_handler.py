...
class TestWordPressHandler:
    ...
    def test_build_wordpress_header_parts_with_title_and_url(self):
        ...
        from markdownall.core.handlers.wordpress_handler import (
            _build_wordpress_header_parts,
        )

        soup = BeautifulSoup("<html><head><title>Test Title</title></head></html>", "html.parser")

        with patch(
            "markdownall.core.handlers.wordpress_handler._extract_wordpress_title",
            return_value="Test Title",
        ):
            with patch(
                "markdownall.core.handlers.wordpress_handler._extract_wordpress_metadata",
                return_value={
                    "author": "Test Author",
                ...
                title, parts = _build_wordpress_header_parts(
                    soup, "https://example.com", "Test Title"
                )
    ...
    def test_fetch_wordpress_article_with_httpx_success(self):
        ...
        with patch("markdownall.core.handlers.wordpress_handler._try_httpx_crawler") as mock_httpx:
            ...
            result = fetch_wordpress_article(
                self.mock_session, "https://example.com/wordpress-post"
            )
    ...
    def test_fetch_wordpress_article_with_httpx_failure(self):
        ...
        with patch("markdownall.core.handlers.wordpress_handler._try_httpx_crawler") as mock_httpx:
            ...
            with patch(
                "markdownall.core.handlers.wordpress_handler._try_playwright_crawler"
            ) as mock_pw:
                mock_pw.return_value = Mock(
                    success=True, html_markdown=long_valid_html, title=None, error=None
                )
    ...
    def test_fetch_wordpress_article_all_strategies_fail(self):
        ...
        with patch("markdownall.core.handlers.wordpress_handler._try_httpx_crawler") as mock_httpx:
            ...
            with patch.dict(
                "sys.modules", {"playwright": fake_pkg, "playwright.sync_api": fake_sync_api}
            ):
                with patch(
                    "markdownall.core.handlers.wordpress_handler._try_playwright_crawler",
                    return_value=Mock(
    ...
    def test_fetch_wordpress_article_with_retry(self):
        ...
        with patch(
            "markdownall.core.handlers.wordpress_handler._try_httpx_crawler",
            side_effect=httpx_strategy_side_effect,
        ):
            result = fetch_wordpress_article(
                self.mock_session, "https://example.com/wordpress-post"
            )
    ...
    def test_fetch_wordpress_article_with_shared_browser(self):
        ...
        with patch("markdownall.core.handlers.wordpress_handler._try_httpx_crawler") as mock_httpx:
            ...
            with patch(
                ...
                result = fetch_wordpress_article(
                    self.mock_session, "https://example.com/wordpress-post", shared_browser=Mock()
                )
                assert result.success is True
    ...
    def test_fetch_wordpress_article_with_stop_requested(self):
        ...
        with patch("httpx.Client", side_effect=StopRequested()):
            with pytest.raises(StopRequested):
                fetch_wordpress_article(
                    self.mock_session, "https://example.com/wordpress-post", should_stop=should_stop
                )
    ...
    def test_try_playwright_crawler_with_shared_browser(self):
        ...
        with patch(
            ...
            with patch("markdownall.services.playwright_driver.teardown_context_page"):
                with patch(
                    "markdownall.core.handlers.wordpress_handler.read_page_content_and_title",
                    return_value=("<html><body><h1>Test</h1></body></html>", "Test Title"),
                ):
                    result = _try_playwright_crawler(
                        "https://example.com/wordpress-post", shared_browser=mock_shared_browser
                    )
    ...
    def test_process_wordpress_content_success(self):
        ...
        result = _process_wordpress_content(
            html, "https://example.com/wordpress-post", "Test Article"
        )
    ...
    def test_build_wordpress_content_element_success(self):
        ...
        from markdownall.core.handlers.wordpress_handler import (
            _build_wordpress_content_element,
        )
    ...
    def test_clean_and_normalize_wordpress_content(self):
        ...
        from markdownall.core.handlers.wordpress_handler import (
            _clean_and_normalize_wordpress_content,
        )
