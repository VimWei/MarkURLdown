...
def test_fetch_weixin_article_success(monkeypatch):
    ...
    wxres = wx.CrawlerResult(success=True, title="T", text_content="<html><div>ok content</div></html>")
    monkeypatch.setattr(wx, '_try_playwright_crawler', lambda url, on_detail=None, shared_browser=None: wxres)
    r = wx.fetch_weixin_article(session=object(), url="https://mp.weixin.qq.com/s/abc", on_detail=None, shared_browser=None)
