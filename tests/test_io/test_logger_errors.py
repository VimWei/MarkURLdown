...
from markdownall.io.logger import _project_root, log_urls


@pytest.mark.unit
def test_log_urls_swallows_io_errors(tmp_path):
    with mock.patch("markdownall.io.logger._project_root", return_value=str(tmp_path)):
        # Make open raise to simulate IO failure
        with mock.patch("markdownall.io.logger.open", side_effect=OSError("disk full")):
            # Should not raise
            log_urls(["https://x"])
        # No file created
