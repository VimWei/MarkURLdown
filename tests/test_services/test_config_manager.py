...
from markdownall.config.config_models import BasicConfig, WebpageConfig, AdvancedConfig, AboutConfig
...
class TestConfigManager:
    ...
    def test_import_config_exception_handling(self):
        ...
        with patch("builtins.open", mock_open(read_data=json.dumps(config_data))):
            with patch.object(self.config_manager, 'set_all_config', side_effect=Exception("Test error")):
    ...
    def test_set_all_config(self):
        ...
        assert self.config_manager.about.homepage_clicked == False  # AboutConfig doesn't have version attribute

    def test_set_all_config_partial(self):
        """Test set_all_config method with partial config."""
        config_data = {
            "basic": {"urls": ["https://partial.com"]},
            "webpage": {"use_proxy": True}
        }
