...
def test_worker_emit_detail_handles_str_and_dict(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("markitdown_app.io.writer.write_markdown", lambda out_dir, fn, text: str(tmp_path / "out.md"))
    ...
    monkeypatch.setattr("markitdown_app.services.convert_service.registry_convert", _reg_convert_side_effect)
    ...
    monkeypatch.setattr("markitdown_app.core.handlers.generic_handler.convert_url", lambda payload, session, options: _reg_convert_side_effect(payload, session, options))
    # Patch the re-export used inside core.registry as well
    monkeypatch.setattr("markitdown_app.core.registry.convert_url", lambda payload, session, options: _reg_convert_side_effect(payload, session, options))
    ...
    assert any(getattr(e, "key", None) == "stage" and getattr(e, "data", None) == {"step": 1} for e in detail_events)
